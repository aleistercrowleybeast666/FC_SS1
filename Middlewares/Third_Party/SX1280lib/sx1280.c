 ...
 */
// { Address, RegValue }
#define RADIO_INIT_REGISTERS_VALUE  { { 0xFFFFU, 0x00U } }
...
{
    for( int16_t i = 0; i < (int16_t)( sizeof( RadioRegsInit ) / sizeof( RadioRegisters_t ) ); i++ )
    {
        if( RadioRegsInit[i].Addr == 0xFFFFU )
        {
            break;
        }
...
{
    uint8_t sf_index = 0U, bw_index = 0U;

    switch( sf )
    {
        case LORA_SF5:  sf_index = 0U; break;
        case LORA_SF6:  sf_index = 1U; break;
        case LORA_SF7:  sf_index = 2U; break;
        case LORA_SF8:  sf_index = 3U; break;
        case LORA_SF9:  sf_index = 4U; break;
        case LORA_SF10: sf_index = 5U; break;
        case LORA_SF11: sf_index = 6U; break;
        case LORA_SF12: sf_index = 7U; break;
        default:
            return 0.0;
    }
    ...
    switch( bw )
    {
        case LORA_BW_0400: bw_index = 0U; break;
        case LORA_BW_0800: bw_index = 1U; break;
        case LORA_BW_1600: bw_index = 2U; break;
        case LORA_BW_0200:
        default:
            return 0.0;
    }

    return RangingCorrectionPerSfBwGain[sf_index][bw_index][gain];
}
//测距相关函数，改动过，现在暂时不用
double SX1280ComputeRangingCorrectionPolynome( const RadioLoRaSpreadingFactors_t sf, const RadioLoRaBandwidths_t bw, const double median )
{
    ...
    polynome = RangingCorrectionPolynomesPerSfBw[sf_index][bw_index];

    for( uint8_t order = 0U; order < polynome->order; order++ )
    {
        correctionCoeff = polynome->coefficients[order] * pow( median, polynome->order - order - 1U );
